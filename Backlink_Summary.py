...
FILENAME = 'Backlink Summary'+TODAY.strftime('%d-%m-%Y')+'.csv'


csvfile1= open('/opt/Alps_Automation/Backlink_Summary.csv', 'wb')
csvWriter1 = csv.writer(csvfile1, delimiter=',')
csvWriter1.writerow(['Fields', 'Expected', 'Actual', 'Result'])

CONTSUM1_EXP='http://aut1.smallbizvoices.com/alps/dashboard/iquanti/projects/17/backlinksummaryaggregate?domain=http://www.shutterstock.com'
CONTSUM1_ACT='http://qa5.smallbizvoices.com/alps/dashboard/iquanti/projects/17/backlinksummaryaggregate?domain=http://www.shutterstock.com'
...
if(f!=0):
    csvWriterN.writerow(['Backlink Summary has ' 'Failed'])
if(f==0):
    csvWriterN.writerow(['Backlink Summary has ' 'Passed'])
...
key.set_contents_from_filename('/opt/Alps_Automation/Backlink_Summary.csv')

os.remove('/opt/Alps_Automation/Backlink_Summary.csv')
