...
describe('As a KRT user', () => {
    ...
    it('AL-T1719: Verify user is able to apply ALL filter in the include filter section.', () => {
        loginAction.clickAlpsLogo()
        krtHome.clickPlanningAndResearch()
        krtHome.clickKeywordResearch()
        krtHome.enterRelatedKeyword(data.testDataKeywordKRT)
        krtHome.clickSearch()
        krtSearch.clickAllFilter()
        krtSearch.addKeywordFilter(data.addKeywordInFilter)
        krtSearch.clickApplyButthon()
        krtSearch.verifyAllFilter(data.addKeywordInFilter)
    })
    it('AL-T1720: Verify user is able to apply ANY Filter in the include filter section.', () => {
        loginAction.clickAlpsLogo()
        krtHome.clickPlanningAndResearch()
        krtHome.clickKeywordResearch()
        krtHome.enterRelatedKeyword(data.testDataKeywordKRT)
        krtHome.clickSearch()
        krtSearch.clickAnyFilter()
        krtSearch.addKeywordFilter(data.addKeywordInFilter)
        krtSearch.clickApplyButthon()
        krtSearch.verifyAllFilter(data.addKeywordInFilter)
    })
