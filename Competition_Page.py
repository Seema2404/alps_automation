...
FILENAME = 'Competition_Page'+TODAY.strftime('%d-%m-%Y')+'.csv'


csvfile1= open('/opt/Alps_Automation/Competition_Page.csv', 'wb')
csvWriter1 = csv.writer(csvfile1, delimiter=',')
csvWriter1.writerow(['Fields', 'Expected', 'Actual', 'Result'])

CONTSUM1_EXP='http://aut1.smallbizvoices.com/alps/dashboard/iquanti/projects/17/projectkeywordsrollup?alias=0&month_range=2&keyword_types=non_branded&sort_by=alias_name&&&limit=10&offset=0'
CONTSUM1_ACT='http://qa5.smallbizvoices.com/alps/dashboard/iquanti/projects/17/projectkeywordsrollup?alias=0&month_range=2&keyword_types=non_branded&sort_by=alias_name&&&limit=10&offset=0'
...
try:
 ...
 csvWriterN.writerow(['Competition Page failed due to API response error'])
 csvfileN.close()
...
try:
 ...
 csvfileN= open('/opt/Alps_Automation/Final.csv', 'a')
 csvWriterN = csv.writer(csvfileN, delimiter=',')
 csvWriterN.writerow(['Competition Page failed due to  response parse error'])
 csvfileN.close()
...
if(f!=0):
    csvWriterN.writerow(['Competition Page has ','Failed'])
if(f==0):
    csvWriterN.writerow(['Competition Page has ' 'Passed'])
...
key.set_contents_from_filename('/opt/Alps_Automation/Competition_Page.csv')

os.remove('/opt/Alps_Automation/Competition_Page.csv')
